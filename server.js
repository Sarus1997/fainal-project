...
app.get('/admin', async (req, res) => {
    ...
    const objects = await client.db('admin').collection('Restaurant')
        .find({}).sort({"Name": 1}).limit(50).toArray();
...
app.get('/admin/membership/restaurants', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('membership').find({"r_role": "restaurant"}).sort({"r_name": 1}).toArray();
    await client.close();
    res.status(200).send(objects);
})
...
app.get('/admin/membership/managers', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('membership').find({"r_role": "manager"}).sort({"r_name": 1}).toArray();
    await client.close();
    res.status(200).send(objects);
})
...
app.get('/ranking/id_quality', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').aggregate([{$group: {_id: "$id_restaurant", id_quality: {$avg: { $toDouble: "$id_quality" }}}}]).toArray();
    for (let i = 0; i < objects.length; i++) {
        objects[i].id_quality = objects[i].id_quality.toFixed(2);
    }
    objects.sort(function (a, b) {
        return b.id_quality - a.id_quality;
    });
    await client.close();
    res.status(200).send(objects);
});
...
app.get('/ranking/id_price', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').aggregate([{$group: {_id: "$id_restaurant", id_price: {$avg: { $toDouble: "$id_price" }}}}]).toArray();
    for (let i = 0; i < objects.length; i++) {
        objects[i].id_price = objects[i].id_price.toFixed(2);
    }
    objects.sort(function (a, b) {
        return b.id_price - a.id_price;
    });
    await client.close();
    res.status(200).send(objects);
});

app.get('/ranking/id_serve', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').aggregate([{$group: {_id: "$id_restaurant", id_serve: {$avg: { $toDouble: "$id_serve" }}}}]).toArray();
    for (let i = 0; i < objects.length; i++) {
        objects[i].id_serve = objects[i].id_serve.toFixed(2);
    }
    objects.sort(function (a, b) {
        return b.id_serve - a.id_serve;
    });
    await client.close();
    res.status(200).send(objects);
});

app.get('/ranking/id_dress', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').aggregate([{$group: {_id: "$id_restaurant", id_dress: {$avg: { $toDouble: "$id_dress" }}}}]).toArray();
    for (let i = 0; i < objects.length; i++) {
        objects[i].id_dress = objects[i].id_dress.toFixed(2);
    }
    objects.sort(function (a, b) {
            return b.id_dress - a.id_dress;
        }
    );
    await client.close();
    res.status(200).send(objects);
...
app.get('/ranking/id_cleanliness', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').aggregate([{$group: {_id: "$id_restaurant", id_cleanliness: {$avg: { $toDouble: "$id_cleanliness" }}}}]).toArray();
    for (let i = 0; i < objects.length; i++) {
        objects[i].id_cleanliness = objects[i].id_cleanliness.toFixed(2);
    }
    objects.sort(function (a, b) {
            return b.id_cleanliness - a.id_cleanliness;
        }
    );
    await client.close();
    res.status(200).send(objects);
...
app.get('/ranking/id_polite', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').aggregate([{$group: {_id: "$id_restaurant", id_polite: {$avg: { $toDouble: "$id_polite" }}}}]).toArray();
    for (let i = 0; i < objects.length; i++) {
        objects[i].id_polite = objects[i].id_polite.toFixed(2);
    }
    objects.sort(function (a, b) {
            return b.id_polite - a.id_polite;
        }
    );
    await client.close();
    res.status(200).send(objects);
...
app.get('/ranking/all', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').aggregate([
        {
            $group: {
                _id: "$id_restaurant",
                id_quality: {$avg: {$toDouble: "$id_quality"}},
                id_price: {$avg: {$toDouble: "$id_price"}},
                id_serve: {$avg: {$toDouble: "$id_serve"}},
                id_dress: {$avg: {$toDouble: "$id_dress"}},
                id_cleanliness: {$avg: {$toDouble: "$id_cleanliness"}},
                id_polite: {$avg: {$toDouble: "$id_polite"}}
            }
        }
    ]).toArray();
    for (let i = 0; i < objects.length; i++) {
        ...
        objects[i].id_cleanliness = objects[i].id_cleanliness.toFixed(2);
        objects[i].id_polite = objects[i].id_polite.toFixed(2);
        objects[i].total = (parseFloat(objects[i].id_quality) + parseFloat(objects[i].id_price) + parseFloat(objects[i].id_serve) + parseFloat(objects[i].id_dress) + parseFloat(objects[i].id_cleanliness) + parseFloat(objects[i].id_polite)).toFixed(2);
        objects[i].total = (parseFloat(objects[i].total) / 30).toFixed(2) * 100;
    }
    objects.sort(function (a, b) {
...
app.get('/reshistory/:name', async (req, res) => {
    const client = new MongoClient(uri);
    await client.connect();
    const objects = await client.db('admin').collection('Data').find({id_restaurant: req.params.name}).toArray();
    await client.close();
    res.status(200).send(objects);
});
